...
int main (int argc, char *args[])
{
    ...
    for (int n = 0; n < N; n+=num_frames*4) {
        if ((err = snd_pcm_readi (pcm_handle, buffer, num_frames)) != num_frames) {
            fprintf (stderr, "Read failed (%s)\n", snd_strerror (err));
            return (err);
        }
        fwrite(buffer, 1, num_frames*4, stdout);
    }
